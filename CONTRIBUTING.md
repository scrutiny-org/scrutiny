...
2. download the `scrutiny-web-frontend.tar.gz` for
   the [latest release](https://github.com/scrutiny-org/scrutiny/releases/latest). Extract to a folder named `dist`
...
```
docker build -f docker/Dockerfile . -t ghcr.io/scrutiny-org/scrutiny:local
docker run -it --rm -p 8080:8080 \
-v /run/udev:/run/udev:ro \
--cap-add SYS_RAWIO \
--device=/dev/sda \
--device=/dev/sdb \
ghcr.io/scrutiny-org/scrutiny:local
